...
function AnomalyTab() {
 ...
 const columns: ColumnsType<CandyAnomaly> = [
   ...
   render: (v: string, r: CandyAnomaly) => v === 'spending'
    ? (
     <Tooltip title={`用户消耗了 ${r.promo_net} 颗糖果，但只有 ${r.stripe_in + r.admin_net} 颗有据可查的来源。多消耗了 ${r.spending_excess} 颗。`}>
      <Tag color="volcano" style={{ cursor: 'help' }}>消费超出</Tag>
     </Tooltip>
    ) : (
  ...
  },
  {
   ...
   dataIndex: 'expected',
   width: 90,
   render: (v: number) => <Text>{v.toLocaleString()} 糖果</Text>,
  },
  {
   title: (
    <span>
   ...
   render: (v: number) => <Tag color="red">+{v.toLocaleString()} 糖果</Tag>,
  },
  {
   title: '账目来源',
   key: 'breakdown',
     ...
     <Space direction="vertical" style={{ width: '100%' }}>
      ...
      {noteModal.action === 'deduct' ? (
       <Alert
        type="warning"
        showIcon
        message={`将从该用户账户扣除 ${noteModal.discrepancy} 糖果，恢复到预期余额。用户会收到通知。`}
        description={
         noteModal.anomalyType === 'spending'
          ? '此用户属于「消费超出」类型：钱包已是0，无法扣除。建议使用「标记合法」并在备注说明（如确认是合理历史消费），或进一步封禁账号。'
          : undefined
        }
       />
      ) : (
...
function PackagesTab() {
 ...
 const ratePreview = (row: CandyPackage) =>
  row.amount > 0 ? `1 ${row.currency} ≈ ${(row.candy / row.amount).toFixed(2)} 糖果` : '—'
